...
SANDBOX_INFO_V8 = (
    ...
    "     **ALWAYS INCLUDE non-empty `interpretation` dictionary, without which the dice roll will fail.**\n"
...
STATIC_SYSTEM_INSTRUCTION_TEMPLATE = (
    ...
    "  - **Call `end_turn` tool after you have provided sufficient story/event/beats for the current turn:** If you have narrated enough for a turn but do not call `end_turn` tool, it will come back to you *without any user's input*. In such cases (i.e. you see a lot of assistant messages without user's input), do NOT continue narrating, just call `end_turn` tool.\n"
